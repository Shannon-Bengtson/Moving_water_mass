...
################## Apply NLS function, running through random starting variables between -0.01 and 0.01 until it works..
	...
	while (!success) {
            ...
            nlsfit <- try(nls(d13C ~ output(a, b, c, e, Lat, Ocean_depth), df, weights = df[, 'count']**0.5, start = list(a = runif(1, min = -1, max= 1)/100, b = runif(1, min = -1, max= 1)/100, c = runif(1, min = -1, max= 0)/100, e = runif(1, min = -1, max= 1)/100), control = nls.control(minFactor = min.fact, tol = tol*100, warnOnly = TRUE), upper = list(a = 10000, b = 10000, c = 0, e = 10000)))
#                        nlsfit <- try(nls(d13C ~ output(a, b, c, e, Lat, Ocean_depth), df, start = list(a = runif(1, min = -1, max= 1)/100, b = runif(1, min = -1, max= 1)/100, c = runif(1, min = -1, max= 0)/100, e = runif(1, min = -1, max= 1)/100), control = nls.control(minFactor = min.fact, tol = tol*100, warnOnly = TRUE), upper = list(a = 10000, b = 10000, c = 0, e = 10000)))

#               nlsfit <- try(nls(d13C ~ output(a, b, c, e, Lat, Ocean_depth), df, weights = df[, 'age_weights'], start = list(a = 0, b = 0, c = 0, e = 0), control = nls.control(minFactor = min.fact, tol = tol, warnOnly = FALSE)))
