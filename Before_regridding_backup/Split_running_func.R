...
split <- function(model, period, type, d13C, d13C.temp, lat, lon, dep, mask, df, equation) {


    dep_4 <- dep[(dep > 1500) & (dep < 4500)]
...
#    if (type == 'data') {dep_4 <- seq(200, 4200, 100)}
    ...
    for (i in seq(1, nrow(deps), 1)) {
        ...
        output <- dget("split.R")(model, type, period, deps[i, 'dep_4'], d13C, d13C.temp, lat, lon, dep, mask, df, equation)
