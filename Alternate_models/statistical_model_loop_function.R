...
loop_func <- function(input) {     
    ...
    #write.table(data.frame('restrict_proxies' = restrict_proxies, file = 'Data/restrict_proxies.txt', row.names = FALSE, col.names = TRUE)
    
######################## Run the python file which will make the new dataframe

    if (input[,'period'] == 'Holocene') {
        system('ipython averaging_interpolated_Holocene_cores.py') }
    if (input[,'period'] == 'LIG') {
        system('ipython averaging_interpolated_LIG_cores.py') }
