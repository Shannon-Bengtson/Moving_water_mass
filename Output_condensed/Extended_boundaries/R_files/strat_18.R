...
#################################################
    ...
    x_coord_min_N <- which(abs(lat - 50) == (min(abs(lat - 50))))
    ...
    x_coord_max_A <- which(abs(lat + 40) == min(abs(lat + 40)))
...
################## Apply NLS function, running through random starting variables between -0.01 and 0.01 until it works..
	...
	while (!success) {
               ...
               nlsfit <- try(nls(d13C ~ output(a, b, c, e, Lat, Ocean_depth), df, weights = df[, 'count'], start = list(a = runif(1, min = -1, max= 1)/1000, b = runif(1, min = -1, max= 1)/1000, c = runif(1, min = -1, max= 1)/1000, e = runif(1, min = -1, max= 1)/1000), control = nls.control(minFactor = min.fact, tol = tol, warnOnly = TRUE)))
#              nlsfit <- try(nls(d13C ~ output(a, b, c, e, Lat, Ocean_depth), df, weights = df[, 'count'], start = list(a = 0, b = 0, c = 0, e = 0), control = nls.control(minFactor = min.fact, tol = tol, warnOnly = TRUE)))
