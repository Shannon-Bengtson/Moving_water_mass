...
loop_func <- function(input) {     
    ...
    # Load LIG data
    write.table(data.frame('lower' = 120, 'upper' = 125), file = 'Data/year_range.txt', row.names = FALSE, col.names = TRUE)
    system('ipython Sorting_moving_period.py')
    df_LIG <- data.frame(read.table("Data/moving_atl.csv", sep = ',', header = TRUE))

    # Load Hol data
    write.table(data.frame('lower' = 2, 'upper' = 8), file = 'Data/year_range.txt', row.names = FALSE, col.names = TRUE)
    system('ipython Sorting_moving_period.py')
    df_Hol <- data.frame(read.table("Data/moving_atl.csv", sep = ',', header = TRUE))
    ...
    # Find the intersection with out current dataframe
