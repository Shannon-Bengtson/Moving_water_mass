...
loop_func <- function(input) {     
    ...
    cat('From year: ', input[,'lower'], '\n')

    write.table(data.frame('lower' = input[, 'lower'][[1]][1]), file = 'Data/year_range.txt', row.names = FALSE, col.names = TRUE)

######################## Run the python file which will make the new dataframe

    system('ipython Sorting_moving_period_2.py')
    print('Ran ipython')
