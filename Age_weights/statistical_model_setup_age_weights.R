...
dataset <- c('LIG')
equation <- c('tanh')
model <- c('UVic') ######check that the model is regridded.......
period <- c('Holocene')

# Set the time bounds (time slice)
resolution <- 0.1 # The number of years to include in each slice
period.1 <- seq(114, 116, resolution)
period.2 <- seq(121, 123, resolution)
period.3 <- seq(127, 129, resolution)

lower <- append(append(period.1, period.2), period.3)
...
input <- expand.grid(dataset = dataset, equation = equation, model = model, type = type, period = period, lower = lower, reps = repetitions)
...
loop.func <- dget("statistical_model_loop_function_age_weights.R")
